...
CHOSEN_LMAX_CACHE = PIPELINE_OUTPUT_DIR / "lmax_optimization_cache.json"
...
rule choose_lmax: 
    input:
        ...
        zarr=SEGMENTED_ZARR
    output:
        lmax_cache=CHOSEN_LMAX_CACHE 
    params:
        enabled=config["lmax_optimization"].get("enabled", True),
        sample_size=config["lmax_optimization"].get("sample_size", 20),
        lmax_min=config["lmax_optimization"]["lmax_range"][0],
        lmax_max=config["lmax_optimization"]["lmax_range"][1],
        lmax_step=config["lmax_optimization"].get("lmax_step", 4),
        sampling_rate=config["lmax_optimization"].get("sampling_rate", 2.752),
        min_object_size=config["lmax_optimization"].get("min_object_size", 500),
        batch_size=config["lmax_optimization"].get("batch_size", 100),
        error_batch_size=config["lmax_optimization"].get("error_batch_size", 5),
        n5_channel_path=config["segmentation"].get("n5_channel_path", "ch2/s0"),
        log_dir=config["dask"].get("log_dir")
    resources:
        runtime=config["dask"].get("runtime", "04:00:00"),
        project=config["dask"]["cpu_project"],
        queue=config["dask"]["cpu_queue"],
        num_workers=config["dask"]["num_cpu_workers"],
        mem_per_worker=config["dask"].get("cpu_memory", "60G"),
        cores_per_worker=config["dask"].get("cpu_cores", 2),
        resource_spec=config["dask"].get("cpu_resource_spec", "mfree=60G"),
        processes=config["dask"].get("cpu_processes", 2)
    conda:
        "otls-pipeline"
    script:
        "scripts/choose_lmax.py"
