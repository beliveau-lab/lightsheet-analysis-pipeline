...
class ValidateLMAX:
    ...
    def run_analysis(self, mask_path, plot_results=False):
        """Run the workflow. Setup dask cluster, load data, run computation, plot results.
    ...
    def _load_data(self, mask_path):
        """Load objects from mask. Same approach as in feature_extraction.py
        but with a different function to find objects.
        ...
        Returns
        -------
        mask_array: sparse array containing the object mask
        df_bboxes: dataframe containing the object bounding boxes
        """
    ...
    def run_computation(self, mask_array, df_bboxes):
        """ Instantiaties delayed tasks for each object -
        computes the reconstruction error for each object at each lmax value.
        ...
        Returns
        -------
        list of dictionaries containing the object id and reconstruction errors f
        or each lmax value
        """
        logger.info("Creating delayed tasks...")
